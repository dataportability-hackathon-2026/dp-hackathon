...
import { LINEAR_ALGEBRA_CONCEPTS, DATA_SCIENCE_CONCEPTS } from "./fixtures"
...
const slideConceptCoverage = {
  ...
  scorer: ({ input, output }: { input: SlideEvalInput; output: SlideArtifactData }) => {
...
const slideStructure = {
  ...
  description: "Checks that slides have proper structure (intro, content, summary)",
  scorer: ({ output }: { input: SlideEvalInput; output: SlideArtifactData }) => {
    let score = 0
    const total = 3

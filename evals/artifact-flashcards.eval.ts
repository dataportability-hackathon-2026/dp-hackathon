...
const flashcardMinimumCount = {
  ...
  scorer: ({
    ...
    input: FlashcardEvalInput;
    output: FlashcardArtifactData;
  }) => {
...
const flashcardQuality = {
  ...
  description:
    "Checks that fronts are questions and backs are substantive answers",
  scorer: ({
    output,
  }: {
    input: FlashcardEvalInput;
...
evalite<FlashcardEvalInput, FlashcardArtifactData>(
  "Flashcard Artifact Generation",
  {
    ...
    scorers: [
      flashcardConceptCoverage,
      flashcardMinimumCount,
      flashcardQuality,
    ],
  },
);

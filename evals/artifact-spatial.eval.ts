...
import { spatialConnectionValidity, spatialPositionSpread } from "./scorers";
...
const spatialObjectCount = {
  ...
  description:
    "Checks that the model has a meaningful number of objects (4-10)",
  scorer: ({
    output,
  }: {
    input: SpatialEvalInput;
    output: SpatialArtifactData;
  }) => {
...
const spatialBoundsCheck = {
  ...
  scorer: ({
    ...
    const inBounds = output.objects.filter(
      (o) =>
        o.x >= -5 && o.x <= 5 && o.y >= -5 && o.y <= 5 && o.z >= -5 && o.z <= 5,
    );
    return inBounds.length / output.objects.length;
  },
};

evalite<SpatialEvalInput, SpatialArtifactData>("Spatial Artifact Generation", {
  data: () => [
    {
      input: {

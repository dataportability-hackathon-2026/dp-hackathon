...
async function loadScheduleFromDb(scheduleId: string): Promise<ScheduleRow | null> {
  "use step"
  const rows = await db
    .select()
    .from(scheduledTask)
...
function shouldTerminate(schedule: ScheduleRow): boolean {
  if (schedule.status === "cancelled" || schedule.status === "expired") return true
  if (schedule.expiresAt && new Date() >= schedule.expiresAt) return true
  if (schedule.maxExecutions && schedule.totalExecutions >= schedule.maxExecutions) return true
  return false
}
